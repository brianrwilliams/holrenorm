...
\title{Holomorphic renormalization}
...
\section{One-loop regularization}

\section{Chiral anomalies in arbitrary dimension}

\section{Holomorphic deformations and the QME}

\begin{thm}
Let $(V, Q^{hol}, (-,-)^{hol})$ be a free holomorphic theory on $\CC^d$, and suppose $I$ is a \brian{holomorphic deformation}.
Then, the resulting effective family $\{I[L]\}$ satisfies the quantum master equation modulo $\hbar^2$ if and only if $\oint \d^d z I$ is a Maurer-Cartan element of $\oint \Obs_V [[\hbar]] / \hbar^2$.
\end{thm}
